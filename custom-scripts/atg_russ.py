# Process
# =======
# 1) Capture image (wait for latest.jpg to arrive in images directory)
# 2) Edit size of image to 24 * 23 CM
# 3) Convert image to grey
# 4) Save image as with file name of  meter.jpg
...
import shutil
if True:
    # Wait for file to exist in images directory
    # path = "gauge-1.jpg"
    # #path = "/Users/russmckay/python/spot-sdk-master/python/examples/spot_cam/latest.jpg"
    # #isExisting = os.path.exists(path)
    # while not os.path.exists(path):
    #     time.sleep(1)
    # print("New file arrived : "+path)
 
    #latest.jpg has arriced so convert to grey, resize and save as new file
    # read the input image
    # input filename = latest.jpg
    # output filename = resized.jpg
    image = cv2.imread('gauge-1.jpg')
    ...
    cv2.imwrite('./images/grey.jpg',adjusted)
    img = cv2.imread('./images/grey.jpg', cv2.IMREAD_UNCHANGED)
    ...
    dim = (width, height)
    resized = cv2.resize(img, dim, interpolation = cv2.INTER_AREA) # ** resized is the name of the file to be saved **
    cv2.imshow("Resized image2", resized)
    ...
    def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, filename,folder_path):
        ...
        if lines is None:
            print("No lines detected by HoughLinesP. Check the image and thresholding.")
            cv2.imwrite(folder_path + "/" + filename + "-debug-threshold.jpg", dst2)
            return None
        ...
        if not final_line_list:
            print("No valid lines found after filtering. Check the filtering parameters.")
            cv2.imwrite(folder_path + "/" + filename + "-debug-threshold.jpg", dst2)
            return None
        ...
        return new_value
    # *******************************************************************
    # ********  SET FILE NAME HERE (temporary code for debugging ********
    # *******************************************************************
    def main():
        folder_path = "images"
        filename = "resized"
        #filename = "33degSpotCamLarge"
        ...
        if val is None:
            print("Gauge value could not be determined.")
            return
        print ("Current reading: %s %s" %(("%.2f" % val), units))
 
        # Open function to open the file "MyFile1.txt"
        # (same directory) in read mode and
        file1 = open("./images/MyFile.txt", "w")
        file1.write(str(val))
        file1.close
        # os.rename('./images/latest.jpg', './images/archives/latest.jpg')
